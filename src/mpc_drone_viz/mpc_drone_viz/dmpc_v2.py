#!/usr/bin/env python3
"""
Approach 2: Naive Distributed MPC with Soft Collision Avoidance (FIXED)
Each drone solves MPC independently with neighbor positions as parameters.
"""
import numpy as np
import rclpy
...
class NaiveDMPCController(Node):
    """
    Naive Distributed MPC with soft inter-drone collision avoidance.
    Neighbor positions are parameters, not variables, so we can update them each cycle.
    """
    
    def __init__(self, drone_id=0, num_drones=3):
        super().__init__(f'dmpc_controller_{drone_id}')
        
        self.drone_id = drone_id
        self.num_drones = num_drones
        
        # Setup static transform broadcaster (only from drone 0)
        if drone_id == 0:
        ...
        # Inter-drone safety parameters
        self.safety_distance_agent = 0.3  # Minimum desired distance between drones
        self.collision_penalty_weight = 3.0  # Weight of collision avoidance in cost
        self.cohesion_distance = 4.0  # Desired maximum distance to neighbors
        self.cohesion_weight = 0.2  # Weight of cohesion in cost
        
        # Drone-specific state
        start_x = drone_id * 1.5
        self.drone_state = np.array([start_x, 0.0, 0.5, 0.0, 0.0, 0.0])
        
        # Shared target and obstacles
        ...
        self.get_logger().info(f'Naive DMPC Drone {drone_id} initialized')
    
    def create_pose_callback(self, drone_id):
        """Factory function for pose callbacks"""
        def callback(msg):
            pos = np.array([
                msg.pose.position.x,
                msg.pose.position.y,
    ...
    def setup_mpc(self):
        """Setup CasADi MPC with neighbor positions as PARAMETERS"""
        opti = ca.Opti()
        ...
        # Neighbor positions as parameters (will be updated each cycle)
        # Store references for 2 neighbors (for 3-drone case)
        neighbor_positions = []
        for i in range(self.num_drones - 1):
            neighbor_pos = opti.parameter(3)
        ...
        # Soft inter-drone collision avoidance (using parameters)
        collision_cost = 0
        for neighbor_param in neighbor_positions:
            for k in range(self.N + 1):
                drone_pos = X[0:3, k]
                
                # Distance to neighbor
                dist = ca.sqrt((drone_pos[0] - neighbor_param[0])**2 + 
                               (drone_pos[1] - neighbor_param[1])**2 + 
                               (drone_pos[2] - neighbor_param[2])**2)
                
                # Repulsion: penalty if too close
                min_dist = self.safety_distance_agent
                repulsion = ca.fmax(0, min_dist - dist)**2
                collision_cost += self.collision_penalty_weight * repulsion
                
                # Weak cohesion: penalty if too far
                max_dist = self.cohesion_distance
                cohesion = ca.fmax(0, dist - max_dist)**2
                collision_cost += self.cohesion_weight * cohesion
        
        # Total cost
        total_cost = target_cost + effort_cost + obstacle_cost + collision_cost
        ...
        self.U = U
        self.X0 = X0
        self.neighbor_params = neighbor_positions
    ...
    def control_step(self):
        ...
        try:
            ...
            # Set neighbor position parameters
            neighbor_id = 0
            for i in range(self.num_drones):
                if i != self.drone_id:
                    if i in self.drone_positions:
                        self.opti.set_value(
                            self.neighbor_params[neighbor_id],
                            self.drone_positions[i]
                        )
                    else:
                        # Default to far away if neighbor not yet heard from
                        self.opti.set_value(
                            self.neighbor_params[neighbor_id],
            ...
            # Log solver info for drone 0 only
            if self.drone_id == 0:
                return_status = sol.stats()['return_status']
                self.get_logger().info(
                    f'Drone {self.drone_id} | Solver: {return_status} | '
                    f'Pos: [{self.drone_state[0]:.2f}, {self.drone_state[1]:.2f}, {self.drone_state[2]:.2f}] | '
                    f'Control: [{U_opt[0, 0]:.4f}, {U_opt[1, 0]:.4f}, {U_opt[2, 0]:.4f}]'
                )
...
def main(args=None):
    ...
    nodes = []
    for i in range(num_drones):
        node = NaiveDMPCController(drone_id=i, num_drones=num_drones)
        nodes.append(node)
