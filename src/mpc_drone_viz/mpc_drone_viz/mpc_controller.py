...
class MPCDroneController(Node):
    def __init__(self):
        ...
        self.max_vel = 1.0  # Max velocity (m/s)
        self.max_acc = 0.5  # Max acceleration (m/s^2)
        
        # State: [x, y, z, vx, vy, vz]
        self.drone_state = np.array([0.0, 0.0, 0.5, 0.0, 0.0, 0.0])
        self.target = np.array([5.0, 5.0, 1.5])
        self.obstacles = [
            {'center': np.array([2.5, 2.5, 1.5]), 'radius': 0.4},
            {'center': np.array([4.0, 3.0, 2.0]), 'radius': 0.35},
            {'center': np.array([3.0, 3.0, 2.0]), 'radius': 0.35},
        ]
    ...
    def setup_mpc(self):
        ...
        # Cost function: track target position + minimize acceleration
        # The MPC tries to reach the target while minimizing energy
        ...
        opti.minimize(target_cost + effort_cost)
